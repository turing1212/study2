function [MSEinner, PCTVAR, VARinner, turningPoint] = innerCVtest(X, Y, num_repeats, holdoutRate,max_components)
...
PCTVAR = zeros(max_components,num_repeats);
NumofY = size(Y,2) + 1;
...
for r = 1:num_repeats
    ...
    for c = 1:max_components
        ...
        PCTVAR(c,r) = sum(pctVAR(2,:));
        % Compute Var explained
        Y_train_pred = [ones(size(X_train_inner,1),1) X_train_inner]*beta;
        VarExpTotal = sum(var(Y_train_pred)) / sum(var(Y_train_inner));
        VarExpEach = var(Y_train_pred) ./ var(Y_train_inner);
